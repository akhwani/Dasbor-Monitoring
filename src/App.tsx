...
export default function App() {
  ...
  const targetSheetUrl = 'https://docs.google.com/spreadsheets/d/173hx5iqe4RbdaG9k2uNM_KTELMC7w3khJvDMucNYBAk/edit?gid=1585102885#gid=1585102885';
  const [sheetUrl, setSheetUrl] = useState(() => {
    const saved = localStorage.getItem('sheetUrl');
    return saved && saved.includes('173hx5iqe4RbdaG9k2uNM_KTELMC7w3khJvDMucNYBAk') ? saved : targetSheetUrl;
  });
              ...
              <div className="bg-white rounded-3xl p-10 border border-slate-200 shadow-sm">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-amber-50 border border-amber-100 rounded-2xl p-6 flex gap-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-amber-900 mb-1">Cara Mengaktifkan Fitur Simpan (Write)</p>
                      <p className="text-xs text-amber-700 leading-relaxed mb-4">
                        Agar perubahan di dasbor dapat tersimpan kembali ke Google Sheets, Anda perlu memasang Google Apps Script:
                      </p>
                      <ol className="text-xs text-amber-700 list-decimal ml-4 space-y-2">
                        <li>Buka Google Sheet Anda, klik menu <b>Extensions</b> &gt; <b>Apps Script</b>.</li>
                        <li>Hapus semua kode yang ada, lalu tempelkan kode Apps Script (hubungi pengembang untuk kodenya).</li>
                        <li>Klik <b>Deploy</b> &gt; <b>New Deployment</b>. Pilih type <b>Web App</b>.</li>
                        <li>Set <b>Execute as: Me</b> dan <b>Who has access: Anyone</b>.</li>
                        <li>Klik <b>Deploy</b>, lalu salin <b>Web App URL</b> dan tempelkan pada kolom di atas.</li>
                      </ol>
                    </div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                    <p className="text-sm font-bold text-slate-800 mb-2">Kode Google Apps Script</p>
                    <p className="text-xs text-slate-500 mb-4">Salin kode di bawah ini ke Apps Script Anda:</p>
                    <div className="relative group">
                      <pre className="bg-slate-900 text-emerald-400 p-4 rounded-xl text-[10px] overflow-x-auto font-mono leading-relaxed max-h-64">
{`function doPost(e) {
  var data = JSON.parse(e.postData.contents);
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheets()[0]; // Sheet utama
  var historySheet = ss.getSheetByName("History") || ss.getSheets()[1];
  
  if (data.action === 'addHistory') {
    // Update main sheet
    var values = sheet.getDataRange().getValues();
    for (var i = 1; i < values.length; i++) {
      if (values[i][1] == data.update.kode_reg) { // Kolom B (kode_reg)
        sheet.getRange(i + 1, 7).setValue(data.update.status); // Kolom G (status)
        sheet.getRange(i + 1, 10).setValue(data.update.progress); // Kolom J (progress)
        sheet.getRange(i + 1, 11).setValue(data.update.keterangan); // Kolom K (keterangan)
        break;
      }
    }
    // Add to history
    historySheet.appendRow([
      data.history.kode_reg,
      data.history.nama_regulasi,
      data.history.tanggal,
      data.history.keterangan,
      data.history.status
    ]);
